...
def eval_mosei_senti_per_class1(results, truths, num_classes=7, exclude_zero=False):
    ...
    per_class_count = {}
    per_class_f1 = {}
    unique_labels = np.unique(test_truth)

    for label in unique_labels:
        label_indices = test_truth == label
        class_acc = accuracy_score(test_truth[label_indices], test_preds[label_indices])
        sample_count = label_indices.sum()
