...
def initiate(hyp_params, train_loader, valid_loader, test_loader):
    ...
    if hyp_params.aligned or hyp_params.model=='MULT1':
        ctc_criterion = None
...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader):
        ...
        for i_batch, (batch_X, batch_Y, batch_META) in enumerate(train_loader):
            ...
            if hyp_params.use_cuda:
                with torch.cuda.device(0):
                    ...
                    if hyp_params.dataset == 'iemocap' or hyp_params.dataset == 'iemo':
                        eval_attr = eval_attr.long()
                    if hyp_params.dataset == 'sad' or hyp_params.dataset == 'sur':
                        eval_attr = eval_attr.long()
                    if hyp_params.dataset == 'hap' or hyp_params.dataset == 'dis':
                        eval_attr = eval_attr.long()
                    if hyp_params.dataset == 'ang' or hyp_params.dataset == 'fea':
                        eval_attr = eval_attr.long()
    ...
    if hyp_params.dataset == "newmosei":
        eval_mosei_senti(results, truths, True)
        eval_mosei_senti_per_class1(results, truths, True)
    elif hyp_params.dataset == 'mosi':
        eval_mosi(results, truths, True)
    elif hyp_params.dataset == 'iemocap' or hyp_params.dataset == 'iemo':
        eval_iemocap(results, truths)
        eval_iemocap1(results, truths)
    elif hyp_params.dataset == 'ang' or hyp_params.dataset == 'hap':
        eval_single(results, truths, hyp_params)
    elif hyp_params.dataset == 'fea' or hyp_params.dataset == 'sad':
        eval_single(results, truths, hyp_params)
    elif hyp_params.dataset == 'dis' or hyp_params.dataset == 'sur':
        eval_single(results, truths, hyp_params)
...
def initiate_eval(hyp_params, train_loader, valid_loader, test_loader):
    model = getattr(models, hyp_params.model + 'Model')(hyp_params)
    ...
    settings = {'model': model,
                ...
                'ctc_criterion': ctc_criterion,
                'scheduler': scheduler}
    return evaluate_model(settings, hyp_params, train_loader, valid_loader, test_loader)


def evaluate_model(settings, hyp_params, train_loader, valid_loader, test_loader):
    criterion = settings['criterion']
    ...
    if hyp_params.dataset == "newmosei" or hyp_params.dataset == "mosei_anger":
        eval_mosei_senti_per_class1(results, truths, True)
        eval_mosei_senti(results, truths, True)
    elif hyp_params.dataset == 'mosi':
        eval_mosi(results, truths, True)
